...
const FileInput = ({ handleFileChange, label, flag = "false" }) => {
    return (
      <div>
        ...
        <div className={`flex ${flag === "true" ? "flex-col" : "md:flex-row "} items-center justify-between border border-gray-300 rounded-lg px-4 py-3 bg-[#EFFFF7] space-y-3 md:space-y-0`}>
          ...
          <label className={`${flag === "true" ? "bg-gray-200" : "bg-[#34FF9D]"} text-black text-sm py-1 px-5 rounded-md cursor-pointer hover:bg-green-500 transition duration-200`}>
            Choose File
